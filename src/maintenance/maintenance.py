...
def execute_table_maintenance(table_name: str) -> str:
    ...
    try:
        # 1. Compaction
        spark.sql(f"CALL local.system.rewrite_data_files(table => 'db.{table_name}')")
        
        # 2. Cleanup (Expire old snapshots)
        spark.sql(f"""
            CALL local.system.expire_snapshots(
                table => 'db.{table_name}', 
                older_than => TIMESTAMP '2030-01-01 00:00:00', 
        ...
        return f"SUCCESS: Maintenance executed for {table_name}. Small files crushed and old snapshots expired. Please run get_table_health to verify."
