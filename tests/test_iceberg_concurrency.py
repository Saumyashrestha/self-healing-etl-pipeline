...
def log(msg):
    ...
    # NEW: Write the timeline to a physical file the LLM can read
    os.makedirs("logs", exist_ok=True)
    with open("logs/simulation_history.log", "a") as f:
...
def run_worker_a():
    ...
    try:
        ...
        crash_time = datetime.now().strftime('%H:%M:%S.%f')[:-3]
        log(f"[Worker A] CRASHED! Iceberg rejected the commit due to an OCC Validation Conflict!")
        
        os.makedirs("logs", exist_ok=True)
        with open("logs/occ_error.log", "w") as f:
            f.write(f"CRASH TIMESTAMP: {crash_time}\n")
            f.write(f"TECHNICAL ERROR: {str(e)}\n")
...
if __name__ == "__main__":
    ...
    # NEW: Clear the old history log before starting a new run
    os.makedirs("logs", exist_ok=True)
    if os.path.exists("logs/simulation_history.log"):
        os.remove("logs/simulation_history.log")
