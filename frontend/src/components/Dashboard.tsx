...
export default function Dashboard({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  ...
  const handleSimulation = async () => {
    ...
    try {
      ...
      await simClient.callTool({ name: "run_incremental_load", arguments: { batches: 50 } });
    } catch (e) { 
      console.error(e);
    } finally { 
  ...
  const chartData = activeDataset.map((dataPoint, index, arr) => {
    const prevPoint = index > 0 ? arr[index - 1] : null;
    // If files dropped significantly, label this event as 'Compaction'
    const isMaintenance = prevPoint && dataPoint.files < prevPoint.files;
    
    return {
      eventName: isMaintenance ? 'Compaction' : (dataPoint.batch ? `Batch ${dataPoint.batch}` : `Step ${index + 1}`),
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-7xl mx-auto">
        ...
        <div className="flex bg-slate-200 p-1 rounded-lg w-fit">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${
              selectedTable === 'orders' ? 'bg-white shadow text-blue-600' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            Orders Table
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${
              selectedTable === 'order_items' ? 'bg-white shadow text-purple-600' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            Order Items Table
          </button>
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* Dynamic Graph */}
        <div className="col-span-12 lg:col-span-8 bg-white p-6 rounded-xl border shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={chartData} margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
              ...
              <XAxis dataKey="eventName" axisLine={false} tickLine={false} tick={{fill: '#94A3B8', fontSize: 11}} dy={10} />
              ...
              {/* Dynamic Line Colors based on selected table */}
              <Line 
                type="monotone" 
                name="Total Files" 
                dataKey="files" 
